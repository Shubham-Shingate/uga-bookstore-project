...
if((nameOnCard.length > 0 && cardNumber.length > 0 && cardExpiry.length > 0 && cvv.length > 0 && cardType.length > 0) || (
	nameOnCard.length == 0 && cardNumber.length == 0 && cardExpiry.length == 0 && cvv.length == 0 && cardType.length == 0
)) {
	return true;
}
else {
	error.innerHTML = "Please complete each field for payment or \n Leave all fields for payment as blank"
	return false;
}
}
...
if((city.length > 0 && state.length > 0 && street.length > 0 && zipcode.length > 0 ) || (
	city.length == 0 && state.length == 0 && street.length == 0 && zipcode.length == 0 
)) {
	return true;
}
else {
	error.innerHTML = "Please complete each field for address or \n Leave all fields for address as blank\n \t"
	return false;
}
}
